...
def LOAD_VIDEO_FRAMES(path, start_frame=None, end_frame=None, EXTRACT_ALL_FRAMES=True, MAX_FRAMES = 1000):
    print(f"Maximum number of frames: {MAX_FRAMES}. Edit it with caution, it uses a lot of memory!!!",)
    frames={}
    ...
    if EXTRACT_ALL_FRAMES:
        ...
        if not cap.isOpened():
            ...
            frame_number = 0
            while frame_number < end_frame and frame_number - start_frame < MAX_FRAMES:
                ret, frame = cap.read()  # Cap frame
                if not ret:
...
def DISPLAY_FRAMES(frames, start_frame=None, end_frame=None, max_col=5, DISPLAY_ALL_FRAMES=False):
    if DISPLAY_ALL_FRAMES:
        ...
        frames_range = {frame_number: frame for frame_number, frame in frames.items() if start_frame <= frame_number <= end_frame}
...
def UPDATE_LABEL_DF(init_lab, end_lab, label_name_in, label_measure_in, data_frame_in):

    # Check if END is Greater than Length
    for index_x in range(init_lab, end_lab+1):
        idx_return_str = data_frame_in['label_measures'][index_x]
        dict_current = ast.literal_eval(idx_return_str)
    
        # Insert Updating DICT
        dict_current.update ({label_name_in: label_measure_in})
        
        # Put Dict into the Current DATA FRAME
        data_frame_in.loc[index_x, 'label_measures'] = str(dict_current)
