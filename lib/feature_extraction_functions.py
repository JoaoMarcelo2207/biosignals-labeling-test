...
def extract_function_bz (capture, VIDEO_ID, TIME_STEP_FR):
    LANDMARK_FILE= os.path.join("..", "shape_predictor_68_face_landmarks.dat")
    ...
    for i, current_compl_path_csv in enumerate(var_FILE_ALL_in):
        ...
        if True:
            ...
            if not capture.isOpened():
                print("!!! Error opening the video (cv2.VideoCapture(VIDEO_RAW_URL) did not return the capture of the video): " + VIDEO_RAW_URL)

            # Extract the Features
            VIDEO_STREAMING_PD = extract_function_bz(capture, VIDEO_ID, TIME_STEP_FR)
            print("VIDEO_STREAMING_PD", VIDEO_STREAMING_PD)
