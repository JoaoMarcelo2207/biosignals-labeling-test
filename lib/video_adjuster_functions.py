...
def find_discontinuities_frames(video_feature_rest, N_FRAMES):    
    ...
    # Filter Data frame to find discontinuous intervals
...
def interpolate_frames(current_path_location, VD_FEATURES_FILE, N_FRAMES=5, OUTPUT_FEATURE_NAME = 'VD_FEATURES_L2.CSV'):
    ...
    FRAMES_DTFRAME, FRAMES_TO_INTERPOLATE, ALL_DISCONTINUITIES = find_discontinuities_frames(video_feature_rest, N_FRAMES)
...
# 03 - Inserting rotation angles =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

def ref3DModel():
    modelPoints = [[ 0.00, -0.24, 0.136], # Sellion
                   [ 0.00,  0.00, 0.000], # Nose
                   [ 0.00,  0.6,  0.100], # Menton
                   [-0.48, -0.21, 1.000], # Right ear
                   [ 0.48, -0.21, 1.000], # Left ear
                   [-0.31, -0.21, 0.400], # Right eye right corner
                   [ 0.31, -0.21, 0.400], # Left eye left corner
                   [ 0.0,   0.22, 0.071]  # Stomion
    ]                                     
    return np.array(modelPoints, dtype=np.float64)

def ref2dImagePoints(shape):
    imagePoints = [shape[27], shape[30], 
                   shape[8], shape[0],
                   shape[16], shape[36], 
                   shape[45], (shape[62]+shape[66])/2]
...
def tuple_to_np(shape_in):
    shape_out = np.zeros([68,2], dtype=float)
    for i in range(0, len(shape_in)):
        shape_out[i] = np.asarray(eval(shape_in.iloc[i]), dtype=float)
    return shape_out

def shape_normalizer(shape):    
    dist_base = 1
    propotion = 0
    ...
    new_shape = transform_scale_updown(shape, propotion)
    return new_shape

def get_angles(image_height, image_width, refImgPts, rotationVector = np.array([None], dtype=object), translationVector = np.array([None], dtype=object), PnP_METHOD = cv2.SOLVEPNP_ITERATIVE):
    # 3Dmodel made by https://github.com/by-sabbir/HeadPoseEstimation
    face3Dmodel = ref3DModel()
    
    fx = 500 *(image_width / 1)
    fy = 500 * (image_height / 1)

    fx = (fx + fy) / 2.0
    fy = fx
    
    cameraMatrix = get_cameraMatrix(fy, (image_width/2, image_height/2))

    mdists = np.zeros((4, 1), dtype=np.float64)
    
    if rotationVector.all() == None:
        # Calculate rotation and translation vector using solvePnP
        success, rotationVector, translationVector = cv2.solvePnP(face3Dmodel, 
                    refImgPts, cameraMatrix, mdists, PnP_METHOD)
    else:
        success, rotationVector, translationVector = cv2.solvePnP(face3Dmodel, 
                    refImgPts, cameraMatrix, mdists, rvec=rotationVector, tvec = translationVector,  useExtrinsicGuess = True, flags=PnP_METHOD)
    
    # Calculating euler angles
    rmat, jac = cv2.Rodrigues(rotationVector)
    angles, mtxR, mtxQ, Qx, Qy, Qz = cv2.RQDecomp3x3(rmat)

    return angles, rotationVector, translationVector
